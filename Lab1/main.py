...
def task2a(start_stop, optimalization_criteria, arrival_time,  *list_stops):
    ...
    path = [Node(stops_dict[0], arrival_time)]
    cost = 0
    cur_arr_time = arrival_time
    for i in range(len(stops_dict)-1):
        print(f'Getting path from {stops_dict[i]} to {stops_dict[i+1]}')
        print(f'Current time: {cur_arr_time}')
        if optimalization_criteria == 't':
            ...
            part_cost, part_path = get_shortest_path_astar_stops(Node(stops_dict[i], cur_arr_time), stops_dict[i+1])
        else:
    ...
    print()
    print_results(start_stop, arrival_time, path, start_stop)
    print(f'Cost: {cost}')
    draw_results(all_connections, path)
    
if __name__ == "__main__":
    # task1b("brücknera", "dubois", datetime.strptime("11:35:00", "%H:%M:%S").time())
    # task1d("kwiska", "pl. grunwaldzki", datetime.strptime("10:00:00", "%H:%M:%S").time())
    task2a('leśnica', 't', datetime.strptime("8:00:00", "%H:%M:%S").time(), 'stabłowice', 'klecina', 'krzyki', 'racławicka', 'biskupin', 'bartoszowice', 'zakrzów', 'kwiska', 'rynek', 'wyszyńskiego')
    # task2a('katedra', 't', datetime.strptime("8:00:00", "%H:%M:%S").time(), 'pl. grunwaldzki', 'hala stulecia')